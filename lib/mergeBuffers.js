...
const BUFFER_MAX_LENGTH = FS_WRITE_MAX_LENGTH;
 ...
 * @private
 */
export function mergeBuffers(gltf, defaultName, force) {
  ...
  forEachBufferViewLikeObject(gltf, function (bufferView) {
    ...
    const sourceBufferViewData = //Buffer.from(
      Buffer.slice(source, // source.slice(
...
function allocateBufferPadding(byteLength) {
  ...
  if (padding > 0) {
    return new Uint8Array(padding); //Buffer.alloc(padding);
  }
  return undefined;
}
